...
class JobsListTableViewCell : UITableViewCell {
    ...
    @IBOutlet var messagebubble: UIImageView!
    func loadItem(#picture: UIImage, message: String, type : String, unread : Bool) {
        ...
        println(unread)
        if (unread){
            self.messagebubble.hidden = false
        }else{
            self.messagebubble.hidden = true
        }
    }
}
...
class JobsListViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(tableView: UITableView,numberOfRowsInSection section:    Int) -> Int {
        return jobs_list.count;
    }
    
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCellWithIdentifier("Jobslisttableviewcellidentifier") as! JobsListTableViewCell
        ...
        if let request = jobs_list[indexPath.row] as? Jobs {
            if (request.requests_pics_urls.count != 0){
                if ((request.requests_pics_urls[0] as! Url).type == "image"){
                    tempimage = UIImage(data: (request.requests_pics_urls[0] as! Url).urldata)!
                }else if((request.requests_pics_urls[0] as! Url).type == "video"){
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject!) {
        if (segue.identifier == "JobsListItemSegue"){
            let index = tableView.indexPathForCell(sender as! JobsListTableViewCell)!
            (segue.destinationViewController as! JobsListItemViewController).IndexPath = index
        }
    }
